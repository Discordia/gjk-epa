...
ExpandingSimplex::ExpandingSimplex(const vector<dvec2>& simplex)
{
...
void ExpandingSimplex::expand(dvec2 point)
{
...
int32_t ExpandingSimplex::calcWinding(const vector<dvec2>& simplex)
{
    size_t size = simplex.size();
    for (int i = 0; i < size; i++)
    {
        ...
        double aCrossB = Vector2Util::cross(a, b);
        if (aCrossB > 0)
        {
            return 1;
        } else if (aCrossB < 0)
